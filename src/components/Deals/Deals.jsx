  ...
  return (
    <div>
      ...
      <div className="flex justify-center items-center gap-3 my-10">
        <button className={`px-4 py-2 ${gradients.primary} text-white font-bold rounded-full hover:cursor-pointer`} onClick={() => setChooseBtn(true)}>
          Products
        </button>
        <button className="px-4 py-2 font-bold rounded-full border border-indigo-500 hover:cursor-pointer" onClick={() => setChooseBtn(false)}>
          Cart (0)
        </button>
      </div>
      ...
      {chooseBtn ? (
        <Suspense fallback={<span className="loading loading-spinner text-primary"></span>}>
          <Card dataPromise={dataPromise} cartItems={cartItems} setCartItems={setCartItems}></Card>
        </Suspense>
      ) : (
