...
function Card({ dataPromise, cartItems, setCartItems }) {
  ...
  const isSelected = (data) => {
    return cartItems.find((p) => p.id === data.id) !== undefined;
  };
        ...
        return (
          <div key={index} className="card max-w-96 bg-base-200 shadow-md ">
            <div className="card-body relative space-y-2">
              <p
                className={`absolute top-4 right-4 rounded-full px-3 py-1 text-[14px] ${badgeColor[data.tagType]}`}
              >
                {data.tag}
              </p>
              ...
              <div className="mt-6 mb-4">
                <button
                  className={`btn ${isSelected(data) ? "bg-indigo-100 text-black" : `${gradients.primary} text-white`} btn-block rounded-full`}
                  onClick={() => handleSubscribe(data)}
                  disabled={isSelected(data)}
                >
                  {isSelected(data) ? "Added to cart" : "Buy Now"}
                </button>
              </div>
            </div>
          </div>
